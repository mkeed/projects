...
const ReaderOpts = struct {
    logging: bool = false,
    name: []const u8 = "",
};
fn readerImpl(comptime opts: ReaderOpts) type {
    return struct {
        const Self = @This();
        data: []const u8,
        idx: usize = 0,
        pub fn takeBytes(self: *Self, len: usize) ![]const u8 {
            if (self.idx + len > self.data.len) {
                return error.TooLong;
            }
            defer self.idx += len;
            if (opts.logging) {
                std.log.info("({s})[{}|{}|{x}]", .{ opts.name, self.idx, len, self.data[self.idx..][0..len] });
            }
            return self.data[self.idx..][0..len];
        }
        pub fn tryGetByte(self: *Self) ?u8 {
...
pub fn ReaderL(comptime name: []const u8) type {
    return readerImpl(.{ .logging = true, .name = name });
}
